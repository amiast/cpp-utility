#ifndef KOTONE_RBST_HPP
#define KOTONE_RBST_HPP 1
...
// Maintains a forest of randomized binary search tree (RBST) with lazy propagation.
...
> struct randomized_binary_search_tree {
  private:
    ...
    void _update(int i) {
        _tree[i].acc = _tree[i].val;
        _tree[i].size = _tree[_tree[i].left].size + _tree[_tree[i].right].size + 1;
        if (int l = _tree[i].left; l) _tree[i].acc = op(_tree[l].acc, _tree[i].acc);
        if (int r = _tree[i].right; r) _tree[i].acc = op(_tree[i].acc, _tree[r].acc);
    }
  ...
  public:
    ...
    randomized_binary_search_tree() : _tree(1) {}

    // Constructs a RBST from the given `vec` and sets `root` to the index of its root.
    randomized_binary_search_tree(int &root, const std::vector<S> &vec) : _tree(1) {
        root = make_tree(vec);
    }
    ...
    // Requires `size > 0`.
    randomized_binary_search_tree(int &root, int size) : _tree(1) {
        assert(size > 0);
        root = make_tree(size);
    }
