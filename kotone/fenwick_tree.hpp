...
template <std::integral S, typename T> struct fenwick_tree {
  private:
    S _len{};
    unordered_map<S, T> _map;
  ...
  public:
    ...
    // Requires `length >= S{}`.
    fenwick_tree(S length) : _len(length + 1) {
        assert(length >= S{});
        assert(length < _len);
    }
...
template <typename T> struct fenwick_tree_2d {
  ...
  public:
    ...
    // Requires `height >= 0`.
    // Requires `width >= 0`.
    fenwick_tree_2d(int height, int width) : _height(height + 1), _width(width), _bits(_height, bit(_width)) {
        assert(height >= 0);
        assert(width >= 0);
    }
